...
func (c *AnyTLSServerConfig) Build() (proto.Message, error) {
	...
	for _, u := range c.Users {
		...
		cfg.Users = append(cfg.Users, &protocol.User{
			Level: uint32(u.Level), Email: u.Email,
			Account: serial.ToTypedMessage(&anytls.Account{Password: u.Password}),
		})
	}
	return cfg, nil
}
...
func (c *AnyTLSClientConfig) Build() (proto.Message, error) {
	...
	return &anytls.ClientConfig{
		Server: &protocol.ServerEndpoint{
			Address: c.Address.Build(), Port: uint32(c.Port),
			User: &protocol.User{
				Level: c.Level, Email: c.Email,
		...
		MinIdleSession:           uint32(c.MinIdleSession),
	}, nil
}
